...
class PurchaseBookkeepingExportsServiceTest extends AbstractServiceTest
{
    ...
    public function testCreate(): void
    {
        ...
        $export = $this->mocoClient->purchaseBookkeepingExports->create($params);
        $this->assertInstanceOf(PurchaseBookkeepingExport::class, $export);
        $this->assertEquals(12345, $export->id);
        $this->assertEquals([123, 234, 345], $export->purchase_ids);

        unset($params['purchase_ids']);
        $this->expectException(InvalidRequestException::class);
        $this->mocoClient->purchaseBookkeepingExports->create($params);
    }
    ...
    public function testGet(): void
    {
        ...
        $this->mockResponse(200, json_encode($this->expectedResponse));
        $export = $this->mocoClient->purchaseBookkeepingExports->get(12345);
        $this->assertInstanceOf(PurchaseBookkeepingExport::class, $export);
        $this->assertEquals("Monthly export for January 2024", $export->comment);

        $this->mockResponse(404);
        $this->expectException(NotFoundException::class);
        $this->mocoClient->purchaseBookkeepingExports->get(999999);
    }
}
